...
def _bootstrap_strategy() -> tuple[str, bool]:
    parser = argparse.ArgumentParser(add_help=False)
    parser.add_argument("--strategy", default="rp_live")
...
def main() -> None:
    strategy_id, list_only = _bootstrap_strategy()
    if list_only:
        _print_strategies()
        return
    selected_preset = get_backtest_strategy(strategy_id)
    available_strategy_ids = [preset.strategy_id for preset in list_backtest_strategies()]

    parser = argparse.ArgumentParser(description="Run multi-timeframe turtle backtest on futures klines.")
    parser.add_argument("--strategy", choices=available_strategy_ids, default=strategy_id)
    parser.add_argument("--list-strategies", action="store_true")
    ...
    preset_defaults = {
        key: value
        for key, value in selected_preset.defaults.items()
    ...
    bars_htf = None
    if need_htf_bars:
        bars_htf, data_source_htf = _load_or_fetch(
            db_path=args.db_path,
            use_binance=args.use_binance,
            symbol=args.symbol,
    ...
    result["data_source_htf"] = data_source_htf
